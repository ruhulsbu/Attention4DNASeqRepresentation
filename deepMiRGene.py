...
class ProbabilityTensor(Wrapper):
 ...
 def call(self, x, mask=None):
  ...
  print("Mask: ", mask)
  print("Cast: ", K.cast(mask, dtype='float32'))
  if mask is not None:
   mask = self.squash_mask(mask)
   p_matrix = make_safe(p_matrix * K.cast(mask, dtype='float32'))
   p_matrix = (p_matrix / K.sum(p_matrix, axis=-1, keepdims=True)) * K.cast(mask, dtype='float32')
  print("Return PT")
  return p_matrix
...
class SoftAttention(ProbabilityTensor):
 ...
 def compute_mask(self, x, mask=None):
  if mask is None or mask.get_shape().ndims == 2:
   return [None, None]
  else:
 ...
 def call(self, x, mask=None):
  ...
  print("Vector: ", p_vector)
  p_vectors = K.expand_dims(p_vector, 2)
  print("Vector: ", p_vectors)
  print("Object: ", x, K.int_shape(x))
...
def main(argv):
 ...
 try:
  ...
  print("deepMiRGene.py -i <inputfile> -o <outfile>")
  sys.exit(2)
 for opt, arg in opts:
  if opt == '-h':
   print("deepMiRGene.py -i <inputfile> -o <outfile>")
 ...
 model.load_weights("./model/new_test.hdf5")
 
 # test
 X_test = one_hot_wrap(import_data("%s" % INFILE), MAX_LEN, DIM_ENC)
 print("-------------------------------------------")
 print("Predicting %d query sequences from \"%s\"." % (X_test.shape[0],INFILE))

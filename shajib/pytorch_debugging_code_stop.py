...
root_dir = "./"
x_data_pos_full, y_data_pos_full = read_input_file(os.path.join(root_dir, "dataset/gene_range_stop_codon.txt"), 1)

original_neg_intergenic_data, original_neg_intergenic_label = read_input_file(os.path.join(root_dir, "dataset/intragenic_stop_codon.txt"), 0)
original_neg_coding_data, original_neg_coding_label = read_input_file(os.path.join(root_dir, "dataset/coding_stop_codon.txt"), 0)
...
saved_model = [model]
torch.save(model.state_dict(), "./pytorch_model_stop.param")
the_model = AttnDecoderRNN(5, 32, 32, device, batch_size=batch_size, debug=0)
the_model.load_state_dict(torch.load("./pytorch_model_stop.param"))

...
root_dir = "./"
x_data_pos_full, y_data_pos_full = read_input_file(os.path.join(root_dir, "dataset/coding_region_sequences"), 1)
x_data_pos_full, y_data_pos_full = randomized_index(x_data_pos_full, y_data_pos_full)

x_data_neg_full, y_data_neg_full = read_input_file(os.path.join(root_dir, "dataset/intrag_region_sequences"), 0)
...
for epoch in range(num_epochs):  # again, normally you would NOT do 300 epochs, it is toy data
    ...
    torch.save(model.state_dict(), "./pytorch_model_coding.param")
   

saved_model = [model]
#torch.save(model.state_dict(), "./pytorch_model_coding.param")
the_model = AttnDecoderRNN(5, 32, 32, device, batch_size=batch_size, debug=0)
the_model.load_state_dict(torch.load("./pytorch_model_coding.param"))

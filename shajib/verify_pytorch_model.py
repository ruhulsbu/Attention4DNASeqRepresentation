...
the_model.load_state_dict(torch.load("./pytorch_model.param"))
the_model = the_model.to(device)
saved_model = [the_model]


X_eval = torch.from_numpy(np.array(x_data_pos_full).astype(int)).to(device)
Y_eval = torch.from_numpy(np.array(y_data_pos_full).astype(int)).to(device)
print(X_eval.shape)
evaluate_model(X_eval, Y_eval, "Independent Positive Test")

print(X_eval.shape)
X_eval = torch.from_numpy(np.array(x_data_neg_full).astype(int)).to(device)
Y_eval = torch.from_numpy(np.array(y_data_neg_full).astype(int)).to(device)
evaluate_model(X_eval, Y_eval, "Independent Negative Test")
